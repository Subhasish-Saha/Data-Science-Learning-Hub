...
"""Splice a content fragment into a shell at the <!--CONTENT_INSERT--> marker.
    python3 insert.py PY_Part01_DataModel.html frag1.html
...
def count(tag, s):
    ...
    return o, c
bad = []
for tag in ("article","div","figure","section","svg","g","text","details","table","nav","p","span","canvas","label","tspan","textarea","pre","button"):
    o, c = count(tag, html)
...
print(f"{sys.argv[1]}: {len(html):,}b · articles={count('article',html)[0]} · "
      f"diagrams={html.count('figure class=\"diagram\"')} · queries={html.count('<div class=\"sqlrun\"')} · "
      f"qa={html.count('<details class=\"qa\"')}")
